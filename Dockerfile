...
COPY docker/runtime-supervisor.sh /usr/local/lib/hivemap/runtime-supervisor.sh
...
HEALTHCHECK --interval=10s --timeout=5s --start-period=20s --retries=5 CMD node --input-type=module -e "const response = await fetch('http://127.0.0.1:8787/health'); if (!response.ok) process.exit(1);"
